...
const AiringTodayTVShows = () => {
    ...
    const fetchAiringTodayTVShows = () => {
        setIsPending(true);
        fetch(`https://api.themoviedb.org/3/tv/airing_today?&page=${page}&api_key=${process.env.REACT_APP_API_KEY}`)
            .then(res => res.json())
            .then(data => {
                console.log(data);
                setTVShows(data.results);
            })
            .catch(error => console.error('Error fetching airing today TV shows:', error))
            .finally(() => {
                setTimeout(() => {
                    setIsPending(false);
                }, 100);
        ...
        <div className='text-white bg-slate-900'>
            ...
            <div className="container">
                ...
                <div className="flex justify-center mt-4">
                    {page > 1 && (
                        <Link to={`/airing-today-tv?page=${page - 1}`}>
